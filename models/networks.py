...
class ResnetGenerator_our(nn.Module):
    ...
    def forward(self, input):
        ...
        #attention1-9是foreground attention mask
        #repeat(1,3,1,1)是把通道数给复制了3份，即channel变成3，以便能和contention相乘
        ...
        #rain_attentionmask = attention1 + ... + attention9
        #attention10 是 background attention mask
        ...
        #attention10是background attention mask
...
class MultiscaleDiscriminator(nn.Module):
    ...
    def __init__(self , input_nc , ndf=64 , n_layers=3 , norm_layer=nn.BatchNorm2d , num_D=2):
        ...
        for i in range(num_D):
            netD = NLayerDiscriminator(input_nc , ndf , n_layers , norm_layer , kw=i*2)
    ...
    def singleD_interfeat(self , model , input):
        result = [input]
        for i in range(len(model)):
            result.append(model[i](input))
    ...
    def forward(self, input):
        ...
        for i in range(num_D):
            ...
            feature_model = [getattr(self, 'scale'+str(num_D-1-i)+'_layer'+str(j)) for j in range(2)]
            interfeat.append(self.singleD_interfeat(feature_model , input_downsample))
            input_downsample =self.downsample(input_downsample)

        return results, interfeat
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d , kw=4):
        ...
        setattr(self , 'model0' , nn.Sequential(*sequence[:8]))
        setattr(self , 'model1' , nn.Sequential(*sequence[:11]))
        # setattr(self , 'model2' , nn.Sequential(*[sequence[:12]]))
