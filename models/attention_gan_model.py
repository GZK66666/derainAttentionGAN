...
class AttentionGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define discriminators
            ...
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.vgg =networks.VGGNet().cuda().eval()
    ...
    def forward(self):
        ...
        self.rec_a1_b, self.rec_a2_b, self.rec_a3_b,self.rec_a4_b, self.rec_a5_b, self.rec_a6_b, self.rec_a7_b, self.rec_a8_b, self.rec_a9_b, self.rec_a10_b, \
    ...
    def backward_G(self):
        ...
        # GAN loss D_A(G_A(A))
        pred_fake_B , interfeat_fake_B = self.netD_A(self.fake_B)
        self.loss_G_A = self.criterionGAN(pred_fake_B, True)
        # GAN loss D_B(G_B(B))
        pred_fake_A , interfeat_fake_A = self.netD_B(self.fake_A)
